...
void testUserMadlibs(const char *aPath, std::ostream &anOutput) {
    ...
    std::unique_ptr<MadlibWordProvider> theProvider(new MadlibUserWordProvider);  //make a USER provider on the hea)p
    std::unique_ptr<MadLibsController> theController(new MadLibsController);  //make a controller on the heap
